...
// gelen veriyi servo motora açı olarak gönderme
...
void setup() 
{
  motor.attach(D4);     						  // motor D4 pinine bağlı        
  motor.write(aci);								  // baslangicta aciyi 0 a ayarla
...
void baglan()
{
  ...
  mqtt_istemci.subscribe("motor");		// motor konusuna abone ol
}
//*********************************************************************
void mesaj_geldi(String &konu, String &veri)		// gelen veriyi oku
{
  Serial.println(konu + ": "+ veri);
  aci=veri.toInt();			// gelen veriyi integer a çevir
  motor.write(aci);			// motor açısını ayarla
}
